...
@section('page_subtitle', 'Kelola pendaftaran, seleksi, diklat, verifikasi berkas, dan keberangkatan calon ABK')
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['total_abk'] }}</strong>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['dokumen_pending'] }}</strong>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['mcu_pending'] }}</strong>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['diklat_aktif'] }}</strong>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['waiting_list'] }}</strong>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['abk_onboard'] }}</strong>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['dokumen_approved'] }}</strong>
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <strong class="text-xl sm:text-2xl font-bold text-slate-900">{{ $stats['dokumen_rejected'] }}</strong>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-200 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-50/50">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('reports.export.pdf') }}" target="_blank" class="bg-white hover:bg-slate-100 border border-slate-300 text-slate-700 px-4 py-2 rounded-xl text-xs font-semibold shadow-sm transition">
                    <i class="fa-solid fa-print mr-1"></i> Cetak Laporan PDF
                </a>
                <a href="{{ route('reports.export.csv') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow-md transition">
            ...
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody id="candidateTableBody" class="divide-y divide-slate-100 bg-white">
                    @forelse($candidates as $candidate)
                        ...
                        @endphp
                        <tr class="candidate-row hover:bg-slate-50/50 transition-colors" data-stage="{{ $candidate->current_tahap }}" data-posisi="{{ $profile ? $profile->posisi_dilamar : '' }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    @endphp
                                    <img class="h-10 w-10 rounded-full object-cover ring-2 ring-slate-100 flex-shrink-0" src="{{ $photo }}" alt="">
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-slate-900 candidate-name">{{ $profile ? $profile->nama_lengkap : $candidate->name }}</div>
                                        <div class="text-xs text-slate-400">{{ $candidate->email }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-800">{{ $profile ? $profile->nomor_hp : '-' }}</div>
                                <div class="text-xs text-slate-400">{{ $profile ? $profile->tempat_lahir : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 py-1 rounded-lg text-xs font-semibold bg-slate-100 text-slate-700">
                                    {{ $profile ? $profile->posisi_dilamar : 'Belum Ditentukan' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-xs font-semibold text-slate-900">
                                {{ $tahapLabel[$candidate->current_tahap] ?? 'Tahap ' . $candidate->current_tahap }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2.5 py-0.5 rounded-full text-xs font-bold uppercase
                                    {{ $candidate->tahap_status === 'Selesai' ? 'bg-emerald-100 text-emerald-800' : '' }}
                                    {{ $candidate->tahap_status === 'Dalam Proses' ? 'bg-blue-100 text-blue-800' : '' }}
                                    {{ $candidate->tahap_status === 'Revisi' ? 'bg-amber-100 text-amber-800' : '' }}
                                    {{ $candidate->tahap_status === 'Pending' ? 'bg-purple-100 text-purple-800' : '' }}
                                    {{ $candidate->tahap_status === 'Terjadwal' ? 'bg-cyan-100 text-cyan-800' : '' }}
                                    {{ $candidate->tahap_status === 'Ditolak' ? 'bg-red-100 text-red-800' : '' }}
                                ">
                                    {{ $candidate->tahap_status }}
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <a href="{{ route('operator.abk.show', $candidate->id) }}" class="inline-flex items-center justify-center bg-slate-900 hover:bg-slate-800 text-white font-semibold px-4 py-2 rounded-xl text-xs shadow-sm transition">
