...
@section('page_subtitle', 'Monitoring keseluruhan sistem rekrutmen dan penempatan calon ABK PT. Samudra Ina Pertiwi')
...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center space-x-4 hover:shadow-md transition">
            <div class="w-12 h-12 rounded-xl {{ $colorMap[$card['color']] }} flex items-center justify-center text-lg flex-shrink-0">
                <i class="fa-solid {{ $card['icon'] }}"></i>
            </div>
            <div class="min-w-0">
                <span class="text-[10px] font-semibold text-slate-400 block uppercase tracking-wider truncate">{{ $card['label'] }}</span>
                <strong class="text-2xl font-extrabold text-slate-900">{{ $card['value'] }}</strong>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div class="lg:col-span-7 bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Jumlah calon ABK yang mendaftar setiap bulan di tahun {{ date('Y') }}</p>
                </div>
                <span class="px-3 py-1 bg-blue-50 text-blue-700 text-xs font-bold rounded-full">{{ date('Y') }}</span>
            </div>
        ...
        <div class="lg:col-span-5 bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            <div class="mt-4 flex items-center justify-center space-x-6 text-xs">
                <span class="flex items-center"><span class="w-3 h-3 rounded-full bg-emerald-500 mr-1.5"></span>Lulus: {{ $mcuStats['Lulus'] }}</span>
                <span class="flex items-center"><span class="w-3 h-3 rounded-full bg-amber-400 mr-1.5"></span>Pending: {{ $mcuStats['Pending'] }}</span>
                <span class="flex items-center"><span class="w-3 h-3 rounded-full bg-red-500 mr-1.5"></span>Gagal: {{ $mcuStats['Gagal'] }}</span>
            </div>
        </div>
    </div>

    {{-- ===== ROW 2: Diklat + Placements ===== --}}
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    {{-- ===== ROW 3: Monitoring + Logs ===== --}}
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">

        {{-- Recent Candidate Monitoring --}}
        <div class="lg:col-span-7 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <a href="{{ route('dashboard.operator') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-800">Lihat Semua &rarr;</a>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-5 py-3.5 text-sm font-semibold text-slate-900">
                                {{ $c->profilAbk ? $c->profilAbk->nama_lengkap : $c->name }}
                            </td>
                            <td class="px-5 py-3.5 text-xs text-slate-700">
                                <span class="font-semibold text-blue-700">{{ $tAktif }}.</span>
                                {{ $tahapLabel[$tAktif] ?? 'Tahap '.$tAktif }}
                            </td>
                            <td class="px-5 py-3.5">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase
                                    {{ $tStatus==='Selesai' ? 'bg-emerald-100 text-emerald-800' : '' }}
                                    {{ $tStatus==='Dalam Proses' ? 'bg-blue-100 text-blue-800' : '' }}
                                    {{ $tStatus==='Revisi' ? 'bg-amber-100 text-amber-800' : '' }}
                                    {{ $tStatus==='Terjadwal' ? 'bg-cyan-100 text-cyan-800' : '' }}
                                    {{ $tStatus==='Ditolak' ? 'bg-red-100 text-red-800' : '' }}
                                ">{{ $tStatus }}</span>
                            </td>
                            <td class="px-5 py-3.5">
                                <a href="{{ route('operator.abk.show', $c->id) }}" class="text-xs font-semibold text-slate-600 hover:text-blue-600 transition">
        ...
        <div class="lg:col-span-5 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                ...
                <a href="{{ route('admin.logs') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-800">Lihat Semua &rarr;</a>
            </div>
                ...
                <div class="flex items-start space-x-3 px-5 py-4 hover:bg-slate-50/50 transition">
                    ...
                    <div class="min-w-0">
                        <p class="text-xs text-slate-800 leading-relaxed">
                            <strong class="font-semibold">{{ $log->user ? $log->user->name : 'Sistem' }}</strong>
                            — {{ $log->aktivitas }}
                        </p>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <a href="{{ route('admin.users') }}" class="bg-slate-900 hover:bg-slate-800 text-white p-5 rounded-2xl flex items-center space-x-4 shadow-lg transition group">
        ...
        <a href="{{ route('admin.logs') }}" class="bg-gradient-to-br from-blue-700 to-blue-900 hover:from-blue-600 hover:to-blue-800 text-white p-5 rounded-2xl flex items-center space-x-4 shadow-lg transition group">
        ...
        <a href="{{ route('reports.export.pdf') }}" target="_blank" class="bg-gradient-to-br from-emerald-700 to-emerald-900 hover:from-emerald-600 hover:to-emerald-800 text-white p-5 rounded-2xl flex items-center space-x-4 shadow-lg transition group">
        ...
        <a href="{{ route('reports.export.csv') }}" class="bg-gradient-to-br from-violet-700 to-violet-900 hover:from-violet-600 hover:to-violet-800 text-white p-5 rounded-2xl flex items-center space-x-4 shadow-lg transition group">
...
new Chart(regCtx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            data: {{ json_encode(array_values($registrationsChartData)) }},
            fill: true,
...
new Chart(mcuCtx, {
    ...
    data: {
        ...
        datasets: [{
            data: [{{ $mcuStats['Lulus'] }}, {{ $mcuStats['Pending'] }}, {{ $mcuStats['Gagal'] }}],
...
new Chart(diklatCtx, {
    ...
    data: {
        ...
        datasets: [{
            label: 'Peserta',
            data: [{{ $diklatStats['Lulus'] }}, {{ $diklatStats['Gagal'] }}, {{ $diklatStats['Aktif'] }}],
