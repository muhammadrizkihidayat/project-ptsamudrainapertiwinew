...
<body onload="window.print()">
    ...
    <div class="summary">
        <div class="summary-box">
            <span class="value">{{ $stats['total_abk'] }}</span>
            <span class="label">Total Calon ABK</span>
        </div>
        <div class="summary-box">
            <span class="value">{{ $stats['abk_onboard'] }}</span>
            <span class="label">ABK On Board</span>
        </div>
        <div class="summary-box">
            <span class="value">{{ $stats['abk_berangkat'] }}</span>
            <span class="label">Proses Keberangkatan</span>
        </div>
        <div class="summary-box">
            <span class="value">{{ $stats['waiting_list'] }}</span>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <strong>{{ $c->profilAbk ? $c->profilAbk->nama_lengkap : $c->name }}</strong><br>
                    <span style="color:#64748b; font-size:9px;">{{ $c->email }}</span>
                </td>
                <td>{{ $c->profilAbk ? $c->profilAbk->posisi_dilamar : '-' }}</td>
                <td>Tahap {{ $tAktif }} - {{ $tahapLabel[$tAktif] ?? '' }}</td>
                <td><span class="badge {{ $badgeClass }}">{{ $tStatus }}</span></td>
