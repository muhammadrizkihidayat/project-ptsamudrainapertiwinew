...
@section('page_subtitle', 'Kelola akun super admin, operator, dan calon ABK')

@section('page_actions')
<a href="{{ route('admin.users.create') }}" class="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white font-bold px-5 py-2.5 rounded-xl text-sm shadow-lg shadow-blue-600/30 transition">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody id="usersTableBody" class="divide-y divide-slate-100 bg-white">
                ...
                @endphp
                <tr class="user-row hover:bg-slate-50/50 transition" data-role="{{ $u->role }}" data-status="{{ $u->status_akun }}">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3">
                                <div class="text-sm font-bold text-slate-900 user-name">{{ $u->name }}</div>
                                <div class="text-xs text-slate-400">{{ $u->email }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $roleColor[$u->role] ?? 'bg-slate-100 text-slate-600' }}">
                            {{ $roleLabel[$u->role] ?? $u->role }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $u->status_akun==='aktif' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800' }}">
                            {{ $u->status_akun }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center space-x-2">
                            <a href="{{ route('admin.users.edit', $u->id) }}" class="bg-blue-50 hover:bg-blue-100 text-blue-700 px-3 py-1.5 rounded-lg text-xs font-semibold transition">
